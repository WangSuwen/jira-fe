    ...
    mounted () {
        this.$nextTick(() => {
            setTimeout(() => {
                ...
                let hasDecimals=  this.hasDecimals;
                let endVal = res.val;
                this.unit = res.unit;
                let demo = {};
                this.demo = demo = new CountUp(this.idName, this.startVal, endVal, hasDecimals=="true"?this.decimals:this.withOutDecimals, this.duration, this.options);
                if (!demo.error) {
                    demo.start();
                }
            }, this.delay);
